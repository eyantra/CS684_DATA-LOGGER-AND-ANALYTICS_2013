...
Calendar.prototype._init = function (firstDayOfWeek, date) {
	...
	for (var i = 0; i < 6; ++i, row = row.nextSibling) {
		...
		if (this.weekNumbers) {
            ...
            cell.innerHTML = monthStartDate.getWeekNumber();
...
Date.prototype.getWeekNumber = function() {
    if (Date.useISO8601WeekNumbers) {
        ...
        return this._getWeekNumberStandard();
    }
};


/* Returns the number of the week in the year using Jan 01 as the first week */
Date.prototype._getWeekNumberStandard = function() {
	...
	// Find the number of days the date is away from the calendar year start
	var dayOffset = Math.ceil((date.getTime()-jan1.getTime()) / MS_IN_A_DAY);
	if (dayOffset < 0 && dayOffset >= (-1 * jan1DayOfWeek)) {
		weekNum = 1;
	} else {
		weekNum = 1;
        // copy jan 1 date
	    var testDate = new Date(jan1.getTime());
