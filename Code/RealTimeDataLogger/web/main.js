...
function validateEnrollForm(){
    ...
    if(gn=="-1") {
     ...
     if(!isStringInRange(en,3,30) || !validateName(en)) {
...
function validateTeacherUpdateForm() {
    ...
    if(!isStringInRange(desg1,2,20) || !validateName(desg1)) {
        document.getElementById('desError').style.display = "inline";
        result = false;
    }
    if(!isStringInRange(qual1,2,20) || !validateName(qual2)) {
...
if (!validateNumeric(mr) || mr=="" || mr.length>10){
    document.getElementById('marksError').style.display = "inline";
     result=false;
}
if (!validateNumeric(nmr) || nmr=="" || nmr.length>10){
...
if(!isStringInRange(ans,1,15) || !validateName(ans) ) {
...
var fd=document.topicform.fl_desc.value;
          ...
          if(!isStringInRange(dm,2,20) || !validateName(dm)){
              document.getElementById('tpnameError').style.display = "inline";
             result = false;
              
          }
          if(!isStringInRange(fd,1,100) || !validateName(fd)){
...
if(!isStringInRange(s1,3,30) || !validateName(s1)){
