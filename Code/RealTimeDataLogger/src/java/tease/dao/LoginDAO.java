...
public class LoginDAO {
    ...
    public Login getLogin(String theUserName) {
         Login login = null;
         if(DBConnection.isClosed(con))
            con = DBConnection.getConnection();
            //System.out.println("Here");
         if(!DBConnection.isClosed(con)) {
                 try {
                 String query = "select * from login_data where user_name = '" + theUserName + "'";
                //create a statement
                i++;
                pstmt = con.prepareStatement(query);
                res = pstmt.executeQuery();
                if(res.next()) {
                    login = new Login();
                    login.setUserId(res.getInt("user_id"));
                    login.setUserName(res.getString("user_name"));
